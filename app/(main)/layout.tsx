 ...
 * 오른쪽 칸은 내 프로필 요약(`components/profile/ProfileSummary.tsx`)이다.
 *
...
export default async function MainLayout({ children }: LayoutProps<"/">) {
	...
	return (
		<div className="mx-auto flex w-full max-w-295 flex-1 md:gap-6 md:pr-6 lg:pl-6 xl:px-0">
			...
			{/* 위 76px은 가운데 칸 제목줄(md:pt-6, 30px 제목, md:pb-4)과 같은 높이다 */}
			<aside className="sticky top-0 w-75 shrink-0 self-start pt-19 max-xl:hidden">
				<Suspense>
					<ProfileSummary me={profile} />
				</Suspense>
			</aside>
		</div>
	);
