...
- [x] 서버 응답 전에 화면에 반영하는 것은 좋아요만 한다 ([결정 0021](decisions/0021-like-optimistic.md)). 팔로우는 화면당 한 번 누르는 버튼이라 로딩 표시로 충분하고, 댓글은 검증 실패가 잦아 먼저 그렸다 지우는 비용이 커서 응답을 기다린다
...
- [x] 위 시나리오를 **배포 URL에서 사람 손으로** 한 번 통과
