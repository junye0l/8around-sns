...
Pretendard 하나를 쓴다. `next/font/local`로 `app/fonts/`의 가변 폰트 woff2를 불러오고, 대체 글꼴은
`-apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", system-ui, sans-serif`다 ([결정 0044](decisions/0044-renewal-open-decisions.md)).
아래 굵기는 가변 폰트의 굵기 축으로 낸다.
...
| 폭 | 뼈대 |
|---|---|
| 768px 미만 | 하단 탭 5칸(전체, 팔로잉, 새 글, 좋아요, 프로필), 제목줄 오른쪽에 더 보기 |
| 768 ~ 1023px | 왼쪽 76px 레일. 아이콘 아래 `caption` 라벨이 늘 보인다 |
| 1024px 이상 | 왼쪽 220px 메뉴(아이콘과 라벨), 칸 사이 24px, 가운데 600px. 좌우 여백 24px을 더한 892px이 최대 폭이고 화면 가운데에 선다. 오른쪽 칸은 없다 ([결정 0050](decisions/0050-remove-profile-summary.md)) |
...
- 닫기 확인: 보낼 수 있는 상태(위 disabled 조건의 반대)에서 닫으려 하면 작은 모달로 묻는다. 취소 버튼, 바깥 누르기, Esc, 시트 끌어내리기 모두 같다
  - 제목 `headline` "작성 중인 글을 버릴까요?", 버튼 secondary "계속 쓰기"와 danger "버리기"를 나란히, 첫 포커스는 "계속 쓰기"
  - 게시글 작성, 수정, 댓글, 답글이 대상이다. 프로필 편집은 묻지 않는다 ([결정 0044](decisions/0044-renewal-open-decisions.md))
...
### 전체 `/` · 팔로잉 `/following` · 좋아요 `/likes`

- 제목줄 `large-title`, 768px 미만은 오른쪽에 더 보기
- 첫 탭 이름은 "전체"다. 실제 정렬이 최신순이라 "추천"이라 부르지 않는다 ([결정 0044](decisions/0044-renewal-open-decisions.md))
- 전체만 맨 위에 글쓰기 줄
- 글 카드 목록, 카드 사이 12px
- 로딩: 카드 모양 스켈레톤 3장
- 빈 상태
...
| 화면 | 제목 | 설명 | 버튼 |
|---|---|---|---|
| 전체 | 아직 올라온 글이 없어요 | 첫 글을 남겨보세요. | 첫 글 쓰기 (글쓰기 시트) |
| 팔로잉, 팔로우한 사람 없음 | 아직 팔로우한 사람이 없어요 | 전체 글에서 관심사가 맞는 사람을 찾아 팔로우해 보세요. | 전체 글 둘러보기 |
| 팔로잉, 글 없음 | 아직 새 글이 없어요 | 팔로우한 사람들이 아직 글을 안 썼어요. 그동안 전체 글을 둘러보세요. | 전체 글 둘러보기 |
| 좋아요 | 아직 좋아요한 글이 없어요 | 마음에 드는 글에 하트를 눌러 보세요. | 전체 글 둘러보기 |
...
- 행 아래 `primary` 글자 한 줄만 둔다. 답글이 있으면 "답글 N개 보기", 없으면 "답글 달기". 말풍선 숫자 링크를 따로 두지 않는다 ([결정 0047](decisions/0047-reply-entry-without-replies.md))
...
- 빈 상태: "아직 팔로워가 없어요" / "아직 팔로우한 사람이 없어요", 버튼 전체 글 둘러보기
...
- 버튼은 무엇이 일어나는지 동사로 쓴다. "게시", "팔로우", "전체 글 둘러보기"
